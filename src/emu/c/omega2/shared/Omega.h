...
typedef struct{
    ...
    // 0xA00000-0xBEFFFF: available as RAM (AROS custom RAM banks).
    // CIAState and Chipstate are now separate static allocations outside the
    // Amiga address space — see Memory.c g_chipstate_buf / g_ciastate_buf.
    uint8_t AROSCustomRAM[2031616]; // 0xA00000-0xBEFFFF (replaces old CIAState+Chipstate+Reserved1)
