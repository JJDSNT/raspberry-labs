...
use crate::platform::bootargs::{apply_bootargs, BootTarget};
...
pub extern "C" fn kernel_main(dtb_ptr: usize) -> ! {
    ...
    let mut target = BootTarget::Demo(DemoKind::Flame);
    ...
    match bootargs {
        Some(args) => {
            ...
            apply_bootargs(args, &mut config, &mut target);
        }
        None => {
            log!("BOOT", "No bootargs, using defaults");
        }
    }
    ...
    if config.width  == 0 { config.width  = 320; }
    if config.height == 0 { config.height = 240; }
    if config.width  as usize > MAX_WIDTH  { config.width  = MAX_WIDTH  as u32; }
    if config.height as usize > MAX_HEIGHT { config.height = MAX_HEIGHT as u32; }

    log!("BOOT", "Config: {}x{}x{}", config.width, config.height, config.depth);
    ...
    match Framebuffer::init(config.width, config.height, config.depth) {
        Some(fb) => {
            ...
            match target {
                BootTarget::Diag(d) => {
                    log!("BOOT", "Selected diag: {}", d.as_str());
                    run_diag(d, fb);
                }
                BootTarget::Demo(d) => {
                    log!("BOOT", "Selected demo: {}", d.as_str());
                    run_demo(d, fb);
                }
            }
        }
...
fn panic(info: &PanicInfo) -> ! {
    ...
    if let Some(location) = info.location() {
        log!("PANIC", "at {}:{}:{}", location.file(), location.line(), location.column());
    }
